...
ds_names = [
    "ImgCross-training-data_train",
    "ImgCross-training-data_val",
    "ImgOldImgNew-training-data-1000_train",
    "ImgOldImgNew-training-data-1000_val",
    "ImgOldImgNew-validation-data_val"
]
batch_size = 30
images_ext = ".jpg"
...
def create_ann(image_path, image_to_ann_data, meta):
    labels = []
...
def convert_and_upload_supervisely_project(
    ...
    meta = sly.ProjectMeta(obj_classes=[obj_class_cob, obj_class_ruler])
    ...
    for ds_name, curr_data_path in zip(ds_names, all_folders):
        ...
        images_names = [item for item in os.listdir(curr_data_path) if get_file_ext(item).lower() == images_ext]
        progress = sly.Progress("Create dataset {}".format(ds_name), len(images_names))

        for img_names_batch in sly.batched(images_names, batch_size=batch_size):
            img_pathes_batch = [os.path.join(curr_data_path, im_name) for im_name in img_names_batch]

            img_infos = api.image.upload_paths(dataset.id, img_names_batch, img_pathes_batch)
            img_ids = [im_info.id for im_info in img_infos]

            anns = [create_ann(image_path, image_to_ann_data, meta) for image_path in img_pathes_batch]
